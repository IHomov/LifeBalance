    ...
    <View style={styles.container}>
      ...
      <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.calendar}>
        {dates.map((date) => (
          ...
          >
            <Text style={[styles.dateText, date === 25 && styles.activeDateText]}>{date}</Text>
          </TouchableOpacity>
        ))}
      </ScrollView>
      ...
      <ScrollView showsVerticalScrollIndicator={false}>
        <TaskListItem title="Work Project" time="10:00 AM" status="done" color="#48BB78" />
        <TaskListItem title="UX Design Course" time="04:00 PM" status="In Progress" color="#F6AD55" />
        <TaskListItem title="Family & Kids" time="09:00 PM" status="todo" color="#4299E1" />
      </ScrollView>
    </View>
  );
};
...
const styles = StyleSheet.create({
  ...
  headerTitle: { fontSize: 24, fontWeight: 'bold', color: COLORS.textMain, marginBottom: 20 },
  calendar: { flexDirection: 'row', marginBottom: 30, maxHeight: 60 },
  dateCard: { width: 45, height: 55, justifyContent: 'center', alignItems: 'center', marginRight: 10, borderRadius: 15, backgroundColor: '#EDF2F7' },
  ...
  filters: { flexDirection: 'row', justifyContent: 'space-between', marginBottom: 25 },
